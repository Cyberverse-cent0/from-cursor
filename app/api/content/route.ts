...
import { apiGet } from '@/lib/api/smart-client';
...
let homepageContent = {
  hero: {
    ...
    tagline: 'Dr. Stephen is a senior Lecturer and Head of Department of Psychology at the <a href="https://www.cuea.edu/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline">Catholic University of Eastern Africa</a> with extensive experience in academic strategy, research leadership, and culturally grounded care. Licensed and registered with the <a href="https://www.cpb.or.ke/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline">Kenya Counsellors and Psychologists Board</a>, he advances <a href="/research-hub" className="text-blue-600 hover:text-blue-800 underline">indigenous knowledge systems</a>, <a href="/research-hub" className="text-blue-600 hover:text-blue-800 underline">thanatology</a>, and <a href="/research-hub" className="text-blue-600 hover:text-blue-800 underline">decolonized psychology</a>.',
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Create cache key based on requested section
    const cacheKey = section ? `content:${section}` : 'content:all';

    // Try to get from cache first with smart API client
    const cachedData = await apiGet(`/api/content${section ? `?section=${section}` : ''}`, {
      cacheStrategy: section ? 'dynamic' : 'static',
      priority: 'high'
    });
