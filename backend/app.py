...
from cache.redis_cache import cache, cache_api_response, get_cache_stats
    ...
    try:
        ...
        about_content = {
            "aboutFull": [
                ...
                "Dr. Asatsa has made significant contributions to the field of psychology through his research on cultural contexts and their impact on mental health practices. His work emphasizes the importance of understanding and integrating traditional African knowledge systems into modern psychological frameworks.",
                "As an educator, researcher, and practitioner, Dr. Asatsa continues to bridge the gap between traditional wisdom and contemporary psychological science, ensuring that mental health services in Africa are both culturally relevant and scientifically grounded."
            ],
            "quote": {
        ...
        return jsonify({"detail": "Failed to fetch about content"}), 500
...
def get_research_interests():
    ...
    try:
        ...
        interests = db.list_research_interests(published_only=True)
        return jsonify({"research_interests": interests})
    except Exception as e:
        print(f"Error fetching research interests: {e}")
        return jsonify({"detail": "Failed to fetch research interests"}), 500
...
def get_awards():
    ...
    try:
        from database import db
        awards = db.list_awards(published_only=True)
        return jsonify({"awards": awards})
    except Exception as e:
        print(f"Error fetching awards: {e}")
        return jsonify({"detail": "Failed to fetch awards"}), 500

@app.route("/api/external-profiles", methods=["GET"])
def get_external_profiles():
    """Get external profiles"""
    try:
        from database import db
        profiles = db.list_external_profiles(published_only=True)
        return jsonify({"external_profiles": profiles})
    except Exception as e:
        print(f"Error fetching external profiles: {e}")
        return jsonify({"detail": "Failed to fetch external profiles"}), 500
